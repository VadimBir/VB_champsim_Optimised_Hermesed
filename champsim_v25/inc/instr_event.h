...
struct alignas(64) rob_events_cpu {
    ...
    // v25: NARROWED uint64_t -> uint32_t. Compare-only field: every write
    // goes through PACK_CYCLE (reduces to 27 bits), every read through
    // PCYCLE_*/PACK_CYCLE. Never used as a raw absolute 64-bit cycle.
    // Verified bit-exact-safe (live gaps < HALF_WINDOW=2^26). Footprint: ROB_SIZE*4B.
    uint32_t event_cycle[ROB_SIZE];
