...
void MEMORY_CONTROLLER::schedule(PACKET_QUEUE *queue, uint32_t channel) {
    ...
    // FUSEDSCAN (this campaign): the original code did TWO full SIZE-wide passes —
    // pass 1 found the oldest open-row HIT candidate; only if none existed did
    // pass 2 re-walk the whole array for the oldest row-MISS candidate. Both
    // passes draw their candidate *exclusively* from entries that are non-empty,
    // unscheduled, and bank-not-busy; they differ only in the extra open_row test
    // applied by the hit pass. We fold them into ONE walk: per qualifying entry we
    // update best_miss (always) and best_hit (only when open_row matches). After
    // the walk, selection is identical to the original: if a hit candidate exists
    // it wins (row_buffer_hit=1, oldest = best_hit); otherwise oldest = best_miss.
    // Tie-break is unchanged (strict PCYCLE_LT, ascending index order). This removes
    // an entire 64-entry scan whenever no open-row hit exists (the common case) and
    // skips empty/scheduled holes once all unscheduled entries are seen.
    int hit_index = -1;
    uint64_t hit_cycle = CYC_PACKED_MAX;
    int miss_index = -1;
    uint64_t miss_cycle = CYC_PACKED_MAX;

    // EARLYBRK: only (occupancy - _scheduled) entries are non-empty AND unscheduled.
    // Selection derives *exclusively* from such entries; every other entry hits a
    // `continue`. So once all of them are examined there can be no further candidate
    // in the remainder of the SIZE-wide array — stop early. The skipped DRAM_FULL_DP
    // traces do not affect simulation state/checksum (same basis as the P2 early-out).
    const uint32_t num_unsched = queue->occupancy - _scheduled;
    uint32_t seen = 0;
    ...
    for (uint32_t i=0; i<queue->SIZE; i++) {
        ...
        // non-empty AND unscheduled candidate examined
        seen++;
        ...
        // bank is busy — disqualifies entry for BOTH hit and miss selection
        if (bank_hot.entries[read_channel][read_rank][read_bank].working) {
            ...
            if (seen >= num_unsched) break;
            continue;
        }

        const uint64_t ev = queue->entry[i].event_cycle;

        // miss-candidate pool: every non-empty, unscheduled, bank-free entry
        if ((miss_index == -1) || PCYCLE_LT(ev, miss_cycle)) {
        ...
        // hit-candidate pool: additionally requires the open row to match
        ...
        if ((hit_index == -1) || PCYCLE_LT(ev, hit_cycle)) {
    ...
    // Selection identical to original two-pass logic: hit wins if any exists.
    if (hit_index != -1) {
        oldest_index = hit_index;
        oldest_cycle = hit_cycle;
        row_buffer_hit = 1;
    } else if (miss_index != -1) {
        oldest_index = miss_index;
        oldest_cycle = miss_cycle;
    }
    (void)oldest_cycle;
