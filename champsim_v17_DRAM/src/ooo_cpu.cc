...
public:
    alignas(64) ankerl::unordered_dense::map<uint64_t, ankerl::svector<SQ_FwdInfo, 2>> address_to_entries[NUM_CPUS];
...
void O3_CPU::schedule_instruction()
{
    ...
    auto fused_scan = [=, this](uint32_t start, uint32_t end) -> bool {
        ...
        for (uint32_t w = ws; w < we && w < ROB_WORDS; w++) {
            ...
            uint64_t ready = fc_ptr[w] & cr & ~sinf_p[w] & ~scmp_p[w] & rmask;

            uint64_t stall = ~(fc_ptr[w] & cr) & rmask;
            if (stall) {
...
void O3_CPU::reg_dependency(const uint16_t rob_index) {
    ...
    for (uint32_t j = 0; j < NUM_INSTR_SOURCES; j++) {
        const uint32_t src_reg = ROB.entry[rob_index].source_registers[j];
        if (!src_reg ||
...
void O3_CPU::add_load_queue(const uint16_t rob_index, const uint32_t data_index)
{
    ...
    const auto& robe = ROB.entry[rob_index];
    ROB.entry[rob_index].lq_index[data_index] = lq_index;
    LQ.entry[lq_index].instr_id = robe.instr_id;
    LQ.entry[lq_index].virtual_address = robe.source_memory[data_index];
    LQ.entry[lq_index].ip = robe.ip;
    LQ.entry[lq_index].data_index = data_index;
    LQ.entry[lq_index].rob_index = rob_index;
    LQ.entry[lq_index].asid[0] = robe.asid[0];
    LQ.entry[lq_index].asid[1] = robe.asid[1];
    LQ.entry[lq_index].event_cycle = PACK_CYCLE(current_core_cycle[cpu] + SCHEDULING_LATENCY);
    LQ.occupancy++;
    ...
    const auto* matching_entries = sq_address_map.get_matching_entries(cpu, LQ.entry[lq_index].virtual_address);

    if (matching_entries != nullptr) {
        for (const auto& e : *matching_entries) {
            if ((rob_index != ROB.head) && (LQ.entry[lq_index].producer_id == e.instr_id)) {
                forwarding_index = e.sq_index;
                fwd_info = &e;
                break;
            }

            if ((LQ.entry[lq_index].producer_id == UINT64_MAX) && (LQ.entry[lq_index].instr_id <= e.instr_id)) {
    ...
    if (forwarding_index != SQ.SIZE && fwd_info) {
        if ((fwd_info->fetched == COMPLETED) && (PCYCLE_LE(fwd_info->event_cycle, PACK_CYCLE(current_core_cycle[cpu])))) {
            LQ.entry[lq_index].physical_address = (fwd_info->physical_address & ~(uint64_t)((1 << LOG2_BLOCK_SIZE) - 1)) | (LQ.entry[lq_index].virtual_address & ((1 << LOG2_BLOCK_SIZE) - 1));
...
void O3_CPU::add_store_queue(const uint16_t rob_index, const uint32_t data_index)
{
    ...
    sq_address_map.add_sq_entry(cpu, SQ.entry[sq_index].virtual_address, sq_index, SQ.entry[sq_index].instr_id);
    ...
    SQ.entry[sq_index].event_cycle = PACK_CYCLE(current_core_cycle[cpu] + SCHEDULING_LATENCY);
...
{
    if (RTS0[RTS0_head] >= SQ_SIZE && RTS1[RTS1_head] >= SQ_SIZE &&
        RTL0[RTL0_head] >= LQ_SIZE && RTL1[RTL1_head] >= LQ_SIZE)
        return;
...
int O3_CPU::execute_load(const uint16_t rob_index, const uint32_t lq_index, const uint32_t data_index)
{
    ...
    const auto& lqe = LQ.entry[lq_index];
...
inline void O3_CPU::complete_execution(const uint16_t rob_index)
{
    ...
    const bool has_raw = ROB.entry[rob_index].reg_RAW_producer;
...
void O3_CPU::complete_data_fetch(PACKET_QUEUE *queue, uint8_t is_it_tlb)
{
    ...
    if (is_it_tlb) { // DTLB
        ...
        if (queue->entry[index].type == RFO) {
            ...
            SQ.entry[sq_index].event_cycle = PACK_CYCLE(current_core_cycle[cpu]);
            sq_address_map.update_physical_address(cpu, SQ.entry[sq_index].virtual_address, sq_index, SQ.entry[sq_index].physical_address);
            ...
            LQ.entry[lq_index].translated = COMPLETED;
            LQ.entry[lq_index].event_cycle = PACK_CYCLE(current_core_cycle[cpu]);
...
void O3_CPU::handle_merged_translation(PACKET *provider)
{
    if (provider->store_merged) {
        ITERATE_SET(merged, provider->sq_index_depend_on_me, SQ_SIZE) {
            ...
            SQ.entry[merged].event_cycle = PACK_CYCLE(current_core_cycle[cpu]);
            sq_address_map.update_physical_address(cpu, SQ.entry[merged].virtual_address, merged, SQ.entry[merged].physical_address);
    ...
    if (provider->load_merged) {
        ITERATE_SET(merged, provider->lq_index_depend_on_me, LQ_SIZE) {
            ...
            LQ.entry[merged].event_cycle = PACK_CYCLE(current_core_cycle[cpu]);
...
void O3_CPU::release_load_queue(uint16_t lq_index) {
    ...
    if (LQ.entry[lq_index].ocp_feature) {
        ...
        LQ.entry[lq_index].ocp_feature = NULL;
    }
#endif
    // release LQ entries
