...
class MEMORY_CONTROLLER : public MEMORY {
  public:
    ...
    uint32_t working_bank_count = 0; // Opt-A: O(1) count of banks with working==1 (stats-only signal)
    uint64_t dram_idle_accumulator = 0; // Opt-B: batched count of fully-idle cycles pending LPM advance_idle flush
    ...
    MEMORY_CONTROLLER(const std::string &v1) : NAME(v1.c_str()) {
        ...
        fill_level = FILL_DRAM;
        working_bank_count = 0; // Opt-A: no banks working at construction
        dram_idle_accumulator = 0; // Opt-B: no pending idle cycles at construction
    ...
    void quickReset(){
        ...
        working_bank_count = 0; // Opt-A: all working flags zeroed above
        dram_idle_accumulator = 0; // Opt-B: drop any pending idle cycles on reset
        for (uint32_t c = 0; c < NUM_CPUS; c++) {
            sim_read_access[c] = 0;
            rw_occ[c].both = 0;
        }
    ...
    void schedule(PACKET_QUEUE *queue, uint32_t channel), process(PACKET_QUEUE *queue),
