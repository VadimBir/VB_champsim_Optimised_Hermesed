...
{ //TIME_it("      MM Op");
    ...
    if (all_warmup_complete) {
        ...
        // miss_active — bank is "miss-busy" iff working==1 AND cycle_available>current.
        // cycle_available is now uint64 in a separate array (no 32-bit wrap).
        uint32_t miss_acc = 0;
        const uint64_t cy = current_core_cycle[cpu];
        for (uint32_t ch = 0; ch < DRAM_CHANNELS; ch++)
            for (uint32_t r = 0; r < DRAM_RANKS; r++)
                for (uint32_t b = 0; b < DRAM_BANKS; b++) {
                    miss_acc |= (uint64_t)bank_hot.entries[ch][r][b].working
                              & (uint64_t)(bank_cycle_available[ch][r][b] > cy);
                }
        bool miss_active = (miss_acc != 0);
