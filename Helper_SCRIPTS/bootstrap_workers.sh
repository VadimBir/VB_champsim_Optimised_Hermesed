...
#  4. Smoke quick.sh on each host (parallel), save log
set -u
...
phase_smoke() {
  echo "=== Phase 4: smoke quick.sh parallel ==="
  local CMD='cd /home/cc/champsim_VB && MODEL="no" && ./quickSim/quick.sh --dir champsim_v14 -p 1 --L1 no --L2 no --L3 no --trace LLM256.Py -d 2 -c 2 -bypca --l1byp $MODEL --l2byp $MODEL --l3byp $MODEL --ocp none --repl lru'
  for h in "${HOSTS[@]}"; do
    (
