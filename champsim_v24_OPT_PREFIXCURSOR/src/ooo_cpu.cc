...
void O3_CPU::schedule_instruction() {
    ...
    // PREFIX-CURSOR: dispatch is a strict in-order CONTIGUOUS prefix from the
    // first un-scheduled ROB entry. That frontier is ROB.next_schedule, which
    // do_scheduling already advances to (rob_index+1). Starting the walk there
    // (instead of ROB.head) and stopping at the first entry that is not yet
    // (fetched_complete && event_cycle<=cycle && !sched_inflight && !sched_complete)
    // reproduces the old fused-scan's contiguous dispatch + stall barrier with
    // O(#dispatched + 1) work -- the 8-word P15 dirty fold and the per-word
    // 64-iter cycle_ready build are deleted. do_scheduling stays UNCHANGED, so
    // it keeps advancing ROB.next_schedule, which is exactly our cursor.
    //
    // Cursor invariant: ROB.head <= ROB.next_schedule <= fetch frontier
    // (next_fetch[1]) at all times. retire_rob only drains entries at head that
    // are COMPLETED (hence already scheduled), so head can never pass
    // next_schedule; no cursor reset is needed at retire. On init/flush the ROB
    // reset sets head, next_fetch[*] and next_schedule all to 0 together
    // (block.h reset()/ctor), so the cursor stays consistent there too.
    const uint64_t* __restrict__ fc_l   = rob_events.per_cpu[cpu].fetched_complete;
    const uint64_t* __restrict__ sinf_l = rob_events.per_cpu[cpu].sched_inflight;
    const uint64_t* __restrict__ scmp_l = rob_events.per_cpu[cpu].sched_complete;
    const uint64_t* __restrict__ ec_l   = rob_events.per_cpu[cpu].event_cycle;
    const uint64_t                ccp_l = PACK_CYCLE(current_core_cycle[cpu]);

    const uint32_t cursor = ROB.next_schedule;   // first un-scheduled ROB index
    const uint32_t limit  = ROB.next_fetch[1];   // fetch frontier (exclusive end)

    // Wrap-aware contiguous-prefix walk. Predicate per entry mirrors the old
    // ready/stall semantics exactly: in-range && fetched_complete &&
    // event_cycle<=cycle && !sched_inflight && !sched_complete. do_scheduling(i)
    // advances ROB.next_schedule, so the loop terminates at the stall barrier or
    // at the fetch frontier; cursor resumes there next cycle automatically.
    auto ready_at = [fc_l, sinf_l, scmp_l, ec_l, ccp_l](uint32_t idx) -> bool {
        return BS_TST(fc_l, idx)
            && PCYCLE_LE(ec_l[idx], ccp_l)
            && !BS_TST(sinf_l, idx)
            && !BS_TST(scmp_l, idx);
    };

    if (cursor < limit) {
        // single segment [cursor, limit)
        for (uint32_t idx = cursor; idx < limit; idx++) {
            if (!ready_at(idx))
                return;
            do_scheduling(idx);
        }
    } else {
        // wrap: [cursor, ROB_SIZE) then [0, limit)
        for (uint32_t idx = cursor; idx < ROB_SIZE; idx++) {
            if (!ready_at(idx))
                return;
            do_scheduling(idx);
        }
