...
private:
    ...
    // PRODINLINE: fixed inline buffer `slots[K]` holds the common (small) case
    // entirely on the stack/object, no heap deref. The existing std::vector `vec`
    // remains the OVERFLOW fallback: the moment a list would exceed K elements we
    // migrate the inline contents into `vec` (preserving order, tombstones and the
    // `front` cursor) and from then on use the EXACT original vector path. Thus
    // behavior is bit-identical for ANY K; K only changes how often the heap path
    // is taken. `front` indexes into `slots[0..count)` while inline, or into `vec`
    // after overflow — identical front-cursor semantics in both cases.
    static constexpr uint32_t K = 16;
    struct ProducerList {
        uint32_t front = 0;            // front-cursor: first non-leading-tombstone slot
        uint16_t count = 0;            // # elements in inline `slots` (valid iff !overflow)
        bool overflow = false;         // true => use `vec`; false => use `slots`
        uint16_t slots[K];             // inline ROB indices in insertion (program) order
        std::vector<uint16_t> vec;     // OVERFLOW: ROB indices in insertion order
    };
#ifdef TRUE_SANITY_CHECK
    // TRUE_SANITY_CHECK: diagnostic-only fields. Zero cost when macro is OFF.
    // max_count_seen: branchless cmov watermark updated every append.
    // overflow_events: incremented on every spill_to_vec call.
    uint16_t prodlist_max_count_seen = 0;
    uint64_t prodlist_overflow_events = 0;
#endif
    ...
    uint8_t dirty_regs[256];
    uint32_t dirty_count = 0;
    bool reg_is_dirty[256] = {};
public:
    void init() {
        ...
        for (uint32_t r = 0; r < 256; r++) {
            ...
            reg_producers[r].count = 0;
            reg_producers[r].overflow = false;
            reg_is_dirty[r] = false;
        }
        dirty_count = 0;
        tombstone_count = 0;
        rob_size = ROB_SIZE;
    ...
    // Migrate inline slots -> vec, preserving order/tombstones; `front` unchanged
    // (it already indexes element positions, identical in both representations).
    static inline void spill_to_vec(ProducerList& pl) {
        pl.vec.clear();
        pl.vec.insert(pl.vec.end(), pl.slots, pl.slots + pl.count);
        pl.overflow = true;
    }

    // Append one rob_index, honoring the inline buffer until it would exceed K.
    static inline void append(ProducerList& pl, uint16_t rob_index) {
        if (!pl.overflow) {
            if (pl.count < K) { pl.slots[pl.count++] = rob_index; return; }
            // Inline buffer full -> spill to vec, then append on the vector path.
            spill_to_vec(pl);
        }
        pl.vec.emplace_back(rob_index);
    }
    ...
    void add_producer(uint16_t rob_index, const uint8_t (&destination_registers)[NUM_INSTR_DESTINATIONS_SPARC]) {
        for (uint32_t i = 0; i < NUM_INSTR_DESTINATIONS; i++) {
            if (destination_registers[i] != 0) {
                auto& pl = reg_producers[destination_registers[i]];
                append(pl, rob_index);
#ifdef TRUE_SANITY_CHECK
                // Branchless cmov watermark: compiles to a conditional-move, no branch.
                // Observes the inline count AFTER append (overflow case: count stays K).
                {
                    const uint16_t cur = pl.overflow ? static_cast<uint16_t>(K) : pl.count;
                    prodlist_max_count_seen = (cur > prodlist_max_count_seen) ? cur : prodlist_max_count_seen;
                    prodlist_overflow_events += (uint64_t)pl.overflow; // 1 on first overflow only until next compact
                }
#endif
            }
        }
    ...
    int get_latest_producer(uint32_t reg, uint32_t current_rob, uint32_t rob_head) {
        ...
        if (!pl.overflow) {
            // Inline path: reverse-scan slots[front..count), identical semantics.
            for (uint32_t k = pl.count; k-- > pl.front; ) {
                const uint16_t v = pl.slots[k];
                if (v == TOMBSTONE) continue;
                if ((rob_head <= current_rob) ?
                    ((v >= rob_head) && (v < current_rob)) :
                    ((v >= rob_head) || (v < current_rob))) return v;
            }
            return -1;
        }
        auto& vec = pl.vec;
        ...
        // GLP-HEAD-INVARIANT: (rob_head <= current_rob) depends only on the two
        // params, never on *rit, so it is loop-invariant. Decide once and branch
        // to one of two tight loops. Iteration order, tombstone skip, and the
        // selected/returned producer are identical to the per-element ternary.
        const auto rend_f = vec.rend() - pl.front;
        const bool nowrap = (rob_head <= current_rob);
        if (nowrap) {
            for (auto rit = vec.rbegin(); rit != rend_f; ++rit) {
                if (*rit == TOMBSTONE) continue;
                if ((*rit >= rob_head) && (*rit < current_rob)) return *rit;
            }
        } else {
            for (auto rit = vec.rbegin(); rit != rend_f; ++rit) {
                if (*rit == TOMBSTONE) continue;
    ...
    void remove_producer(uint16_t rob_index, const ooo_model_instr* instr) {
        for (uint8_t i = 0; i < NUM_INSTR_DESTINATIONS; i++) {
            if (uint8_t reg = instr->destination_registers[i]) {
                ...
                auto& pl = reg_producers[reg];
                // Pick the active backing store: inline slots, or the overflow vec.
                // Both expose a contiguous [front..sz) array of rob indices; the
                // tombstone/cursor logic below is byte-for-byte identical either way.
                uint16_t* data;
                uint32_t sz;
                if (!pl.overflow) { data = pl.slots; sz = pl.count; }
                else              { data = pl.vec.data(); sz = pl.vec.size(); }
                ...
                for (uint32_t k = pl.front; k < sz; ++k) {
                    if (data[k] == rob_index) {
                        ...
                        if (!reg_is_dirty[reg]) { reg_is_dirty[reg] = true; dirty_regs[dirty_count++] = reg; }
    ...
    void compact() {
        ...
        // Visiting just the dirty set is identical in effect to the 256-slot sweep.
        for (uint32_t d = 0; d < dirty_count; d++) {
            const uint8_t reg = dirty_regs[d];
            auto& pl = reg_producers[reg];
            if (!pl.overflow) {
                // Inline stable-remove of TOMBSTONEs in slots[0..count).
                uint32_t w = 0;
                for (uint32_t r = 0; r < pl.count; ++r) {
                    if (pl.slots[r] != TOMBSTONE) pl.slots[w++] = pl.slots[r];
                }
                pl.count = static_cast<uint16_t>(w);
            } else {
                auto& vec = pl.vec;
                vec.erase(std::remove(vec.begin(), vec.end(), TOMBSTONE), vec.end());
            }
            pl.front = 0;   // all tombstones removed; reset cursor
            reg_is_dirty[reg] = false;
        }
        dirty_count = 0;
        tombstone_count = 0;
    }
...
#ifdef TRUE_SANITY_CHECK
    // Print watermark/overflow diagnostic. Called from print_end_of_sim_report
    // ONLY when TRUE_SANITY_CHECK is defined. Zero code generated when macro is OFF.
    void print_prodlist_sanity() const {
        std::cout << "*** ProducerList sanity: max inline-slot occupancy = "
                  << prodlist_max_count_seen << " (K=" << K << "). ";
        if (prodlist_max_count_seen > K)
            std::cout << "LIMIT EXCEEDED";
        else
            std::cout << "within K";
        std::cout << ". overflow_events=" << prodlist_overflow_events << " ***" << std::endl;
    }
#endif
};
...
class alignas(64) O3_CPU {
  public:
    ...
    void release_load_queue(uint16_t lq_index);
        ...
        __attribute__((always_inline)) uint32_t check_and_add_lsq(uint16_t rob_index);
