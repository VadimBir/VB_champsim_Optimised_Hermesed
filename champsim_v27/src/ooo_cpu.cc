...
__attribute__((always_inline)) uint32_t O3_CPU::check_and_add_lsq(const uint16_t rob_index) {
...
inline void O3_CPU::complete_execution(const uint16_t rob_index) {
    ...
    // REMPRODSKIP: mirror add_producer's guard — add_producer only inserts for
    // destination_registers[i] != 0. If ALL dest regs are zero (branches, pure
    // loads), add_producer inserted NOTHING, so remove_producer finds nothing and
    // is pure overhead. Skip the call entirely in that case. Bit-exact: the set
    // of tombstoned entries is unchanged (nothing was inserted → nothing to remove).
    {
        bool any_dest_reg = false;
        for (uint32_t _i = 0; _i < NUM_INSTR_DESTINATIONS; _i++)
            any_dest_reg |= (robe.destination_registers[_i] != 0);
        if (any_dest_reg)
