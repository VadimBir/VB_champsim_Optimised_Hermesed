...
uint32_t O3_CPU::add_to_rob(ooo_model_instr *arch_instr) {
    ...
    rob_events.per_cpu[cpu].event_cycle[index] = PACK_CYCLE(current_core_cycle[cpu]);
    // WMIN-SKIP anchor: ROB allocates in-order, so when index is the FIRST bit of
    // its word (index&63==0) all higher (older-live) bits in the word are already
    // retired -> no live sibling can have a lower event_cycle -> re-anchor wmin to
    // current (assignment, sound). Otherwise only LOWER (min). wmin stays a valid
    // lower bound either way; assignment at bit-0 lets the bound rise again so the
    // skip can fire. PACKING OFF -> plain comparisons.
    {
        const uint32_t _w = index >> 6;
        const uint64_t _cc = PACK_CYCLE(current_core_cycle[cpu]);
        if ((index & 63) == 0)
            rob_events.per_cpu[cpu].wmin[_w] = _cc;
        else if (PCYCLE_LT(_cc, rob_events.per_cpu[cpu].wmin[_w]))
            rob_events.per_cpu[cpu].wmin[_w] = _cc;
    }
...
void O3_CPU::fetch_instruction() {
    ...
    for (uint8_t i = 0; i < FETCH_WIDTH; i++) {
        ...
        trace_packet.event_cycle = PACK_CYCLE(current_core_cycle[cpu]);
    ...
    for (uint32_t i = 0; i < FETCH_WIDTH; i++) {
        if ((ROB.entry[fetch_index].translated != COMPLETED) || PCYCLE_GT(rob_events.per_cpu[cpu].event_cycle[fetch_index], PACK_CYCLE(current_core_cycle[cpu])))
            break;
        ...
        fetch_packet.event_cycle = PACK_CYCLE(current_core_cycle[cpu]);
...
void O3_CPU::schedule_instruction() {
    ...
    const uint64_t* __restrict__ wmin_l = rob_events.per_cpu[cpu].wmin;
    const uint64_t                ccp_l = ccp;
    O3_CPU* __restrict__          self  = this;
    auto fused_scan = [fc_l, sinf_l, scmp_l, ec_l, wmin_l, ccp_l, self](uint32_t start, uint32_t end) -> bool {
        ...
        for (uint32_t w = ws; w < we && w < ROB_WORDS; w++) {
            ...
            // WMIN-SKIP: wmin_l[w] is a LOWER bound on event_cycle of every entry in
            // word w. If wmin_l[w] > ccp, EVERY in-range bit has event_cycle > ccp ->
            // cr would be all-0 -> the real scan computes stall = (~(fc&0)&rmask) =
            // rmask != 0 (this word is in-range), truncates ready to 0, returns false.
            // Identical outcome, eliding the O(64) cr build. Stale-low wmin only ever
            // DISABLES this skip (falls through to the unchanged cr-build). PACKING OFF.
            if (PCYCLE_GT(wmin_l[w], ccp_l))
                return false;
...
inline void O3_CPU::do_scheduling(const uint16_t rob_index) {
    ...
    if (is_mem){
        ...
        { uint64_t _e = rob_events.per_cpu[cpu].event_cycle[rob_index], _c = PACK_CYCLE(current_core_cycle[cpu]);
          rob_events.per_cpu[cpu].event_cycle[rob_index] = PACK_CYCLE((PCYCLE_GE(_e,_c) ? _e : _c) + SCHEDULING_LATENCY); }
...
void O3_CPU::do_execution(const uint16_t rob_index) {
    if (BS_TST(rob_events.per_cpu[cpu].reg_ready, rob_index) && BS_TST(rob_events.per_cpu[cpu].sched_complete, rob_index) && PCYCLE_LE(rob_events.per_cpu[cpu].event_cycle[rob_index], PACK_CYCLE(current_core_cycle[cpu]))) {
        ...
        { uint64_t _e = rob_events.per_cpu[cpu].event_cycle[rob_index], _c = PACK_CYCLE(current_core_cycle[cpu]);
          rob_events.per_cpu[cpu].event_cycle[rob_index] = PACK_CYCLE((PCYCLE_GE(_e,_c) ? _e : _c) + EXEC_LATENCY); }
...
void O3_CPU::schedule_memory_instruction() {
   ...
   for (uint32_t w = 0; w < ROB_WORDS; w++)
       ready[w] = im_ptr[w] & rr_ptr[w] & fc_ptr[w] & sinf_p[w] & ~scmp_p[w];
   ...
   auto scan_and_schedule = [&](uint32_t start, uint32_t end) -> bool {
       ...
       for (uint32_t w = ws; w < we && w < ROB_WORDS; w++) {
           ...
           uint64_t mem_fc = im_ptr[w] & fc_ptr[w] & rmask;
           uint64_t mem_bits = im_ptr[w] & rmask;
...
void O3_CPU::add_load_queue(const uint16_t rob_index, const uint32_t data_index) {
    ...
    if (forwarding_index != SQ.SIZE && fwd_info) {
        if ((fwd_info->fetched == COMPLETED) && (PCYCLE_LE(fwd_info->event_cycle, PACK_CYCLE(current_core_cycle[cpu])))) {
            ...
            rob_events.per_cpu[cpu].event_cycle[fwr_rob_index] = PACK_CYCLE(current_core_cycle[cpu]);
            // WMIN-SKIP: store-forward lowering write -> keep wmin a valid lower bound.
            { const uint32_t _w = fwr_rob_index >> 6; const uint64_t _v = PACK_CYCLE(current_core_cycle[cpu]);
              if (PCYCLE_LT(_v, rob_events.per_cpu[cpu].wmin[_w])) rob_events.per_cpu[cpu].wmin[_w] = _v; }
...
void O3_CPU::execute_store(const uint16_t rob_index, uint32_t sq_index, const uint32_t data_index) {
    ...
    rob_events.per_cpu[cpu].event_cycle[rob_index] = PACK_CYCLE(current_core_cycle[cpu]);
    // WMIN-SKIP: execute_store lowering write -> keep wmin a valid lower bound.
    { const uint32_t _w = rob_index >> 6; const uint64_t _v = PACK_CYCLE(current_core_cycle[cpu]);
      if (PCYCLE_LT(_v, rob_events.per_cpu[cpu].wmin[_w])) rob_events.per_cpu[cpu].wmin[_w] = _v; }
    ...
    if (rob_entry->is_producer) {
        ...
        if (pending_lqs != nullptr) {
            ...
            for (uint16_t lq_index : lqs_to_process) {
                ...
                rob_events.per_cpu[cpu].event_cycle[fwr_rob_index] = PACK_CYCLE(current_core_cycle[cpu]);
                // WMIN-SKIP: store-forward (pending load) lowering write -> keep wmin a valid lower bound.
                { const uint32_t _w = fwr_rob_index >> 6; const uint64_t _v = PACK_CYCLE(current_core_cycle[cpu]);
                  if (PCYCLE_LT(_v, rob_events.per_cpu[cpu].wmin[_w])) rob_events.per_cpu[cpu].wmin[_w] = _v; }
...
void O3_CPU::update_rob() {
    ...
    if (L1I.PROCESSED.occupancy && (PCYCLE_LE(L1I.PROCESSED.entry[L1I.PROCESSED.head].event_cycle, PACK_CYCLE(current_core_cycle[cpu]))))
        complete_instr_fetch(&L1I.PROCESSED, 0);

    if (DTLB.PROCESSED.occupancy && (PCYCLE_LE(DTLB.PROCESSED.entry[DTLB.PROCESSED.head].event_cycle, PACK_CYCLE(current_core_cycle[cpu]))))
        complete_data_fetch(&DTLB.PROCESSED, 1);

    if (L1D.PROCESSED.occupancy && (PCYCLE_LE(L1D.PROCESSED.entry[L1D.PROCESSED.head].event_cycle, PACK_CYCLE(current_core_cycle[cpu]))))
        complete_data_fetch(&L1D.PROCESSED, 0);
#ifdef BYPASS_L1_LOGIC
    if (L2C.PROCESSED.occupancy && (PCYCLE_LE(L2C.PROCESSED.entry[L2C.PROCESSED.head].event_cycle, PACK_CYCLE(current_core_cycle[cpu])))) {
...
void O3_CPU::complete_instr_fetch(PACKET_QUEUE *queue, uint8_t is_it_tlb) {
    ...
    rob_events.per_cpu[cpu].event_cycle[rob_index] = PACK_CYCLE(current_core_cycle[cpu]);
    // WMIN-SKIP: fetch-complete lowering write -> keep wmin a valid lower bound.
    { const uint32_t _w = rob_index >> 6; const uint64_t _v = PACK_CYCLE(current_core_cycle[cpu]);
      if (PCYCLE_LT(_v, rob_events.per_cpu[cpu].wmin[_w])) rob_events.per_cpu[cpu].wmin[_w] = _v; }
    num_fetched++;
    ...
    if (queue->entry[index].instr_merged) {
        for (auto i : queue->entry[index].rob_index_depend_on_me) {
            ...
            rob_events.per_cpu[cpu].event_cycle[i] = PACK_CYCLE(current_core_cycle[cpu] + (num_fetched / FETCH_WIDTH));
            // WMIN-SKIP: merged-fetch lowering write -> keep wmin a valid lower bound.
            { const uint32_t _w = i >> 6; const uint64_t _v = PACK_CYCLE(current_core_cycle[cpu] + (num_fetched / FETCH_WIDTH));
              if (PCYCLE_LT(_v, rob_events.per_cpu[cpu].wmin[_w])) rob_events.per_cpu[cpu].wmin[_w] = _v; }
            num_fetched++;
        }
    }
...
void O3_CPU::complete_data_fetch(PACKET_QUEUE *queue, uint8_t is_it_tlb) {
    ...
    if (is_it_tlb) { // DTLB
        ...
        rob_events.per_cpu[cpu].event_cycle[rob_index] = PACK_CYCLE(queue->entry[index].event_cycle);
        // WMIN-SKIP: complete_data_fetch lowering write -> keep wmin a valid lower bound.
        { const uint32_t _w = rob_index >> 6; const uint64_t _v = PACK_CYCLE(queue->entry[index].event_cycle);
          if (PCYCLE_LT(_v, rob_events.per_cpu[cpu].wmin[_w])) rob_events.per_cpu[cpu].wmin[_w] = _v; }
    }
    else { // L1D or ByP L2C
        ...
        if (queue->entry[index].type == RFO)
            ...
            rob_events.per_cpu[cpu].event_cycle[rob_index] = PACK_CYCLE(queue->entry[index].event_cycle);
            // WMIN-SKIP: complete_data_fetch (load) lowering write -> keep wmin a valid lower bound.
            { const uint32_t _w = rob_index >> 6; const uint64_t _v = PACK_CYCLE(queue->entry[index].event_cycle);
              if (PCYCLE_LT(_v, rob_events.per_cpu[cpu].wmin[_w])) rob_events.per_cpu[cpu].wmin[_w] = _v; }
...
void O3_CPU::handle_merged_load(PACKET *provider) {
    ...
    ITERATE_SET(merged, provider->lq_index_depend_on_me, LQ_SIZE) {
        ...
        // WMIN-SKIP: handle_merged_load lowering write -> keep wmin a valid lower bound.
        { const uint32_t _w = merged_rob_index >> 6; const uint64_t _v = PACK_CYCLE(current_core_cycle[cpu]);
          if (PCYCLE_LT(_v, rob_events.per_cpu[cpu].wmin[_w])) rob_events.per_cpu[cpu].wmin[_w] = _v; }
