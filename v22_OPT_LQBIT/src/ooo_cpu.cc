...
//
// LQBIT: the inner per-address set of LQ indices is replaced by a fixed-width
// bitset (LQ_SIZE bits). LQ indices are dense (0..LQ_SIZE-1), so a bitset gives
// O(1) insert/erase with no heap hash-table per address, and range-for via
// __builtin_ctzll. Set membership is identical to emhash7::HashSet<uint16_t>:
//   insert == set bit, erase == clear bit, empty == all words zero,
//   iterate == ascending scan of set bits. IPC-safe.
struct LqBitset {
    static constexpr uint16_t NWORDS = (LQ_SIZE + 63) / 64;
    uint64_t w[NWORDS] = {};

    inline void insert(uint16_t i) { w[i >> 6] |= (1ULL << (i & 63)); }
    inline void erase(uint16_t i)  { w[i >> 6] &= ~(1ULL << (i & 63)); }
    inline bool empty() const {
        uint64_t acc = 0;
        for (uint16_t k = 0; k < NWORDS; k++) acc |= w[k];
        return acc == 0;
    }

    // Forward iterator over set bits in ascending index order (ctz scan).
    struct iterator {
        const uint64_t* w;
        uint16_t word;
        uint64_t cur;   // remaining set bits of current word
        void advance_to_nonempty() {
            while (cur == 0 && word + 1 < NWORDS) { ++word; cur = w[word]; }
        }
        uint16_t operator*() const { return (uint16_t)((word << 6) + __builtin_ctzll(cur)); }
        iterator& operator++() {
            cur &= cur - 1;          // clear lowest set bit
            advance_to_nonempty();
            return *this;
        }
        bool operator!=(const iterator& o) const { return word != o.word || cur != o.cur; }
    };
    iterator begin() const {
        iterator it{w, 0, w[0]};
        it.advance_to_nonempty();
        return it;
    }
    iterator end() const { return iterator{w, NWORDS - 1, 0}; }
};

struct LQ_PendingLoads {
    alignas(64) emhash7::HashMap<uint64_t, LqBitset> address_to_lq[NUM_CPUS];
    ...
    inline LqBitset* get_pending_loads(uint16_t cpu, uint64_t virtual_address) {
...
LqBitset* pending_lqs;
LqBitset lqs_to_process;
