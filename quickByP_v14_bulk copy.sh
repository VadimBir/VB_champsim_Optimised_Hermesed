...
parse_args() {
  ...
  while [[ $# -gt 0 ]]; do
    ...
    --time)
      isTimeBin=1; shift ;;
    -h|--help) echo "Usage: $0 [all quick_v13.sh flags] — sweep mode (no args) or single-run CLI mode"; exit 0 ;;
...
LAUNCH_DELAY_SEC=0.1

# Isolated build temp root
WORK_BASE="/tmp/champsim_builds"
DB_FNAME="${OUT_ROOT:-./champsim_results}/champsim_results_eval_ByP.db"
...
# Each model name should match a file in champsim_v13_qmeta/src/ByP_Models/
...
CHAMPSIM_DIR_NAME="${CHAMPSIM_DIR_NAME:-champsim_v13_qmeta}"
...
if [[ -f "$RERUN_CONFIG_FILE" ]]; then
  ...
  while IFS= read -r _cfg_line; do
    ...
    drain_pending
    wait_for_slot
    echo "STARTED: $currRunCnt out of $totRunCnt | max_now=$(current_max_procs) | running_now=$(count_running_jobs)"
...
# ---- PHASE 2: normal sweep (identical to original script) ----
echo "--- PHASE 2: normal sweep ---"
for cores in "${CORE_LIST[@]}"; do
            for pf_spec in "${PREFETCH_LIST[@]}"; do
    for model_name in "${MODEL_LIST[@]}"; do
        for trace in "${TRACE_LIST[@]}"; do
        ...
        echo "STARTED: $currRunCnt out of $totRunCnt | ccd=$_ccd_cpus | max_now=$(current_max_procs) | running_now=$(count_running_jobs) | $(grep 'cout << ' "$(resolve_model_file "${_l1m:-$model_name}" ".l1_bypass" 2>/dev/null)" 2>/dev/null || true)"
