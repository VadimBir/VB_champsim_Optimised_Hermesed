...
champsimDirName="${champsimDirName:-champsim_v13_qmeta}"
PfBuilder=qbuildPrefetcher_v13
PfRunner=qrun_champsim_v13
BP="${BP:-perceptron}"
REPL="${REPL:-lru}"
...
usage() {
  ...
  echo "  --profile [name] [events] Enable profiling (default: cycles:u → perf.data)"
  echo "  --time                    Time the binary execution (wall + user + sys)"
  echo ""
  echo "Other:"
  echo "  --malloc TYPE             Use custom allocator (mimalloc|jemalloc|none)"
  echo "  --drain <path>            Drain mode: build binary as usual, then APPEND the"
  echo "                            full sim CLI invocation to <path> instead of running it."
  echo "                            Use quick_v13_drainer.sh to consume the drain file."
...
for buf in "${BUF_SZ[@]}"; do
  ...
  for n in $PROCESSES_NUM; do
    ...
    if [ "$isDebug" -eq 2 ]; then
      ...
      output=$(./"$PfBuilder".sh --dir "$champsimDirName" --repl "$REPL" --win-mode "$WIN_MODE" --hermes "$OCP" --l1byp "$L1_BYP_MODEL" --l2byp "$L2_BYP_MODEL" --l3byp "$L3_BYP_MODEL" "$L1" "$L2" "$L3")
      echo "$output"
      status=$?
    else
      output=$(./"$PfBuilder".sh --dir "$champsimDirName" --repl "$REPL" --win-mode "$WIN_MODE" --hermes "$OCP" --l1byp "$L1_BYP_MODEL" --l2byp "$L2_BYP_MODEL" --l3byp "$L3_BYP_MODEL" "$L1" "$L2" "$L3" 2>&1)
      status=$?
    ...
    # --drain mode: append the canonical sim CLI to the drain file instead of executing it
    if [[ -n "${DRAIN_FILE:-}" ]]; then
      DRAIN_CMD="./${PfRunner}.sh --dir ${champsimDirName} --bp ${BP} --repl ${REPL} --win-mode ${WIN_MODE} --hermes ${OCP} --l1byp ${L1_BYP_MODEL} --l2byp ${L2_BYP_MODEL} --l3byp ${L3_BYP_MODEL} ${trace} ${L1} ${L2} ${L3}"
      printf '%s\n' "$DRAIN_CMD" >> "$DRAIN_FILE"
      echo "[drain] Appended to $DRAIN_FILE: $DRAIN_CMD"
      continue
    fi

    if [ "$isDebug" -eq 10 ]; then
          # ./"$PfRunner".sh "$trace" "$L1" "$L2" "$L3" # | grep -E "Finished CPU|now IPC :" # # > /dev/null 2>&1
          echo "./$PfRunner.sh --dir $champsimDirName $trace $L1 $L2 $L3 1"
          ./"$PfRunner".sh --dir "$champsimDirName" --bp "$BP" --repl "$REPL" --win-mode "$WIN_MODE" --hermes "$OCP" --l1byp "$L1_BYP_MODEL" --l2byp "$L2_BYP_MODEL" --l3byp "$L3_BYP_MODEL" "$trace" "$L1" "$L2" "$L3" "1"
          exit 0
    fi
    ...
    for ((i = 0; i < n; i++)); do
      {
        ...
        ./"$PfRunner".sh --dir "$champsimDirName" --bp "$BP" --repl "$REPL" --win-mode "$WIN_MODE" --hermes "$OCP" --l1byp "$L1_BYP_MODEL" --l2byp "$L2_BYP_MODEL" --l3byp "$L3_BYP_MODEL" "$trace" "$L1" "$L2" "$L3"
