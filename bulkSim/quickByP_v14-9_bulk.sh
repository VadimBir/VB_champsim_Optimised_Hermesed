...
REMOTE_HOSTS=(10.52.3.1 10.52.0.126)
...
if [[ -n "${OUT_ROOT:-}" && "$OUT_ROOT" != "./outputs/outTTTEVALS" ]]; then
  mkdir -p "$OUT_ROOT"
...
count_running_jobs() {
  ...
  local n
  n=$($REMOTE_SSH cc@$host "pgrep -c champsim 2>/dev/null || echo 0" 2>/dev/null) || n=999
  [[ "$n" =~ ^[0-9]+$ ]] || n=999
  echo "$n"
}
...
run_one() {
  ...
  # Build trace args (cores copies) using remote path
  local trace_args=""
  ...
  if [[ "$_build_status" == "BUILT" ]]; then
    ...
    local _bin_pid=$!
    rsync -az --ignore-existing -e "$_rsync_ssh" "$trace_file" "cc@$host:${remote_trace_file}" 2>/dev/null &
    local _trace_pid=$!
    wait $_bin_pid || _bin_rc=$?
    wait $_trace_pid || _trace_rc=$?
    ...
    # CACHE HIT — single SSH: mkdir logdir, verify binary+trace, launch
    $REMOTE_SSH cc@$host "mkdir -p '${remote_logdir}' '${remote_trace_dir}' && test -x '${remote_bin}' && test -f '${remote_trace_file}' && nohup bash -c \"$cmd\" >/dev/null 2>&1 &" 2>/dev/null
    local _hit_rc=$?
    if (( _hit_rc != 0 )); then
  ...
  echo "[SENT-${_build_status}] ${PHASE_TAG:-?} | ${cores}c | trace=${trace_hint} | model=${model_tag} | pf=${l1_pf}/${l2_pf}/${l3_pf} | → $host"
  return 0
}
...
sweep_pass() {
  ...
  for _bkey in "${!_built_keys[@]}"; do
    ...
    (
      ...
      if (( _rc != 0 )); then
        ...
        echo "[BUILD-ALL-FAIL] ${_bkey} rc=${_rc}"
      else
        echo "[BUILD-ALL-OK] ${_bkey} → ${_out#*::}"
      fi
    ) &
    _build_pids+=($!)
    sleep 0.5
  ...
  while (( _dq_idx < _dq_total )); do
    ...
    if (( _slots == 0 )); then
      ...
      while (( _slots == 0 )); do
        ...
        if (( _checked >= ${#REMOTE_HOSTS[@]} )); then
          echo "[SEND-ALL] All hosts full — waiting 5s..."
          sleep 5
          _checked=0
        fi
    ...
    echo "[SEND-ALL] Host $_host: ${_running}/${_max} running, ${_slots} slots available"
    ...
    echo "[SEND-ALL] Sent ${_sent} to $_host"
    # Move to next host
    REMOTE_HOST_IDX=$(( (REMOTE_HOST_IDX + 1) % ${#REMOTE_HOSTS[@]} ))
  done
...
# ---- WAIT: poll remote hosts until all sims finish, pull logs periodically ----
if (( DRY_RUN == 0 )); then
  ...
  while true; do
    ...
    echo "[POLL] $(date +%H:%M:%S) remote sims running: $total_running  pending: $pending_lines"

    # Pull logs periodically
    _now=$(date +%s)
    if (( _now - _last_sweep >= SWEEP_OUT_GREP_PERIOD )); then
      sweep_remote_logs
      _last_sweep=$_now
    fi
