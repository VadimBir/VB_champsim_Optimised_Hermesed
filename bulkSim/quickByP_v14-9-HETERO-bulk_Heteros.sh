...
OUT_DIR_SIGNATURE="Hx_outSPEC_HETERO-"
...
sweep_pass() {
  ...
  for host in "${REMOTE_HOSTS[@]}"; do
    (
      ...
      # Collect all remote dirs, create in one SSH call
      local -a _rdirs=("${REMOTE_BASE}/traces" "${REMOTE_BASE}/outputs")
      for _sbin in "${!_sync_bins[@]}"; do
        local _rbin="${REMOTE_BASE}/${_sbin#$ROOT/}"
        _rdirs+=("$(dirname "$_rbin")")
      done
      local _mkdir_args; printf -v _mkdir_args "'%s' " "${_rdirs[@]}"
      $REMOTE_SSH cc@$host "mkdir -p ${_mkdir_args}"

      # Parallel bin rsyncs
      for _sbin in "${!_sync_bins[@]}"; do
        local _rbin="${REMOTE_BASE}/${_sbin#$ROOT/}"
        rsync -az -e "$_rsync_ssh" "$_sbin" "cc@$host:${_rbin}" &
        _rpids+=($!)
      done

      # Parallel trace rsyncs
      local _rtrace_dir="${REMOTE_BASE}/traces"
      for _strace in "${!_sync_traces[@]}"; do
        local _tbase; _tbase="$(basename "$_strace")"
        rsync -az --ignore-existing -e "$_rsync_ssh" "$_strace" "cc@$host:${_rtrace_dir}/${_tbase}" &
        _rpids+=($!)
      done

      # Wait all parallel rsyncs
      for _rp in "${_rpids[@]}"; do
        wait "$_rp" || echo "[SYNC-WARN] rsync failed on $host"
      done
