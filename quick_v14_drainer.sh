#!/bin/bash
# quick_v14_drainer.sh — persistent drain-file consumer for quick_v14.sh --drain queues.
...
#
# Mirrors the bin-name-building logic in quick_v14.sh / qbuildPrefetcher_v14.sh.
...
_jobname() {
  ...
  positional="$(echo "$cmd" \
    ...
    | sed 's/-c[[:space:]]\+[0-9]\+//g' \
    | sed 's/\.[\/]*qrun_champsim_v14\.sh\|\.\/quick_v14\.sh//g')"
  local -a toks=()
