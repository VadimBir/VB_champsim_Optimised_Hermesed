...
uint32_t CACHE::get_way(const uint64_t address, const uint32_t set) const {
    // Scan packed SOA tag mirror (same equivalence as check_hit).
    const uint64_t* tc = &tag_check[(size_t)set * NUM_WAY];
...
void CACHE::fill_cache(const uint32_t set, const uint32_t way, PACKET *packet) {
    ...
    tag_check[(size_t)set * NUM_WAY + way] = packet->address; // SOA mirror: way now VALID
...
int CACHE::check_hit(PACKET *packet) {
    ...
    if (FORCE_ALL_HITS) { //(cache_type == IS_L1D || cache_type == IS_L2C || cache_type == IS_LLC)) {
        ...
        tag_check[(size_t)set * NUM_WAY + 0] = packet->address; // SOA mirror
    ...
    // hit — scan the packed SOA tag mirror (contiguous, ~1 cache line for 8 ways).
    // INVALID ways hold TAG_INVALID, which no real block address equals, so
    // (tc[way] == addr) is exactly (valid && tag == addr) of the original scan.
    const uint64_t* tc = &tag_check[(size_t)set * NUM_WAY];
    const uint64_t addr = packet->address;
    for (uint32_t way=0; way<NUM_WAY; way++) {
        if (tc[way] == addr) {
            match_way = way;
...
int CACHE::invalidate_entry(uint64_t inval_addr) {
    ...
    // invalidate — scan packed SOA tag mirror (same equivalence as check_hit)
    uint64_t* tc = &tag_check[(size_t)set * NUM_WAY];
    for (uint32_t way=0; way<NUM_WAY; way++) {
        if (tc[way] == inval_addr) {
            block[set][way].valid = 0;
            tc[way] = TAG_INVALID;          // SOA mirror: way now invalid
            match_way = way;
...
int CACHE::add_rq(PACKET *packet) {
   ...
   // check if WQ packet has ByPass and new packet not
    ...
    // ADD LATENCY
    // ccp-hoist: single packet->cpu, no intervening writer; factor the raw global read.
    const uint64_t now = current_core_cycle[packet->cpu];
    if (PCYCLE_LT(RQ.entry[index].event_cycle, PACK_CYCLE(now)))
        RQ.entry[index].event_cycle = PACK_CYCLE(now + LATENCY);
    else
...
int CACHE::add_wq(PACKET *packet) {
    ...
    // ccp-hoist: single packet->cpu, no intervening writer; factor the raw global read.
    const uint64_t now = current_core_cycle[packet->cpu];
    if (PCYCLE_LT(WQ.entry[index].event_cycle, PACK_CYCLE(now)))
        WQ.entry[index].event_cycle = PACK_CYCLE(now + LATENCY);
    else
