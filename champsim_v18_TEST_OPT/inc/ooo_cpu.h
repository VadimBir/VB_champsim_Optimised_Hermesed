...
private:
    ...
    ankerl::unordered_dense::map<uint32_t, ProducerList> reg_producers;
    uint16_t rob_size;
public:
    void init() {
        reg_producers.clear();
        reg_producers.reserve(256);
        rob_size = ROB_SIZE;
        ...
        for (uint8_t i = 0; i < NUM_INSTR_DESTINATIONS; i++) {
            if (uint8_t reg = instr->destination_registers[i]) {
                ...
                if (it != reg_producers.end()) {
                    ...
                    for (uint32_t k = pl.front; k < vec.size(); ++k) {
                        if (vec[k] == rob_index) {
                            vec[k] = TOMBSTONE; ++tombstone_count;
                            // Advance the cursor past any now-leading tombstones.
                            while (pl.front < vec.size() && vec[pl.front] == TOMBSTONE)
                                ++pl.front;
                            break;
                        }
                    }
